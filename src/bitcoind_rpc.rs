...
impl Client {
    ...
    async fn get_raw_transaction_rpc<R>(&self, _txid: Txid, _is_verbose: bool) -> Result<R>
    where
        R: std::fmt::Debug + DeserializeOwned,
    {
        todo!()
        // let request = ::jsonrpc_client::Request::new_v1(
        //     "getrawtransaction",
        //     vec![json_rpc::serialize(txid)?, json_rpc::serialize(is_verbose)?],
        // );
        // let request = ::serde_json::to_string(&request)?;
        // let _response = self
        //     .send_request::<R>(request)
        //     .await
        //     .map_err(::jsonrpc_client::Error::Client)?;
        // let success = jsonrpc_client::Result::from(response.payload)
        //     .map_err(::jsonrpc_client::Error::JsonRpc)?;
        // Ok(success)
    }
